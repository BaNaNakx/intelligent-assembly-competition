...
_TRIGGER_TARGETS: Mapping[str, tuple[Color, EntityKind]] = {
    ...
    **{color.tray_trigger: (color, EntityKind.TRAY) for color in Color},
}
...
def parse_vm_trigger_response(trigger: str, payload: bytes) -> VisionMeasurement:
    ...
    'Validate the one fixed VM packet returned for an agent trigger.\n\n    VisionMaster may include a display prefix such as ``格式化结果:`` in its\n    TCP response, but the protocol packet itself must use ``#0;X;Y;RZ;`` and\n    retain the final semicolon shown in the competition example.\n    '
    normalized_trigger = trigger.strip()
    try:
        expected_key = _TRIGGER_TARGETS[normalized_trigger]
    except KeyError as exc:
        raise VmProtocolError("智能体触发字符必须是方块 11-16 或托盘 21-26。") from exc
    ...
    packets = tuple(
        match.group("packet") for match in _VM_TRIGGER_RESPONSE_PATTERN.finditer(text)
    )
    if len(packets) != 1:
        raise VmProtocolError(
            "VisionMaster 每个触发字符必须返回且只返回一条以分号结尾的 #0;X;Y;RZ; 数据。"
        )

    measurement = parse_vm_measurement(packets[0])
    if measurement.key != expected_key:
        expected_color, expected_kind = expected_key
        raise VmProtocolError(
            f"触发字符 {normalized_trigger} 必须返回"
            f"{expected_color.display_name}{expected_kind.value}的固定数据。"
        )
    return measurement
