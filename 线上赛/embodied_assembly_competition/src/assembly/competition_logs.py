...
_STEP_PREFIXES = ("先把", "再把", "接着把", "然后把", "再把", "最后把")
_CHINESE_NUMBERS = ("一", "二", "三", "四", "五", "六")
...
def render_llm_log(snapshot: CompetitionEvidenceSnapshot) -> str:
    ...
    if snapshot.task2_plan is None:
        ...
        for step in snapshot.task2_plan.steps:
            ...
            lines.append(
                f"步骤{number}：{prefix}{step.block_color.display_name}方块放到"
                f"{step.tray_color.display_name}托盘上，"
                f"{step.block_color.display_name.removesuffix('色')}（{step.block_trigger}）→"
                f"{step.tray_color.display_name}（{step.tray_trigger}），{status}。"
            )
    return "\n".join(lines).rstrip() + "\n"
