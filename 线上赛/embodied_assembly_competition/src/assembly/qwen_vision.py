...
class QwenVisionClient:
    ...
    def analyze_task2(
        ...
        if not task1_summary.strip():
            raise ValueError("任务卡 1 场景结论不能为空。")
        prompt = _TASK2_PROMPT.format(task1_summary=task1_summary)
        result, _ = self._request_structured_result(image, prompt, stage="任务二")
...
任务卡 1 的场景结论是：{task1_summary}
只输出 JSON，格式必须为：
{{"instruction":"完整原始中文装配指令"}}
instruction 中必须按图片顺序明确写出六条“颜色方块放到颜色托盘上”的指令；不得使用 Markdown。
""".strip()
