        ...
        for trigger, packet in expected.items():
            with self.subTest(trigger=trigger):
                measurement = parse_vm_trigger_response(
                    trigger,
                    f"格式化结果:{packet}".encode("utf-8"),
                )
                self.assertEqual(measurement.raw_packet, packet)

    def test_trigger_response_rejects_wrong_branch_or_missing_final_semicolon(self) -> None:
        with self.assertRaisesRegex(VmProtocolError, "触发字符 11"):
            parse_vm_trigger_response("11", b"#0;4;-4;40;")
        with self.assertRaisesRegex(VmProtocolError, "分号结尾"):
            parse_vm_trigger_response("11", b"#0;1;-1;10")
