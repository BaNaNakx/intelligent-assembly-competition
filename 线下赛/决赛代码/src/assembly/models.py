...
class Color(str, Enum):
    ...
    def tray_trigger(self) -> str:
        try:
            ...
            raise ValueError(f"{self.display_name}物块没有对应托盘。") from exc

    @property
    def has_tray(self) -> bool:
        return self in _TRAY_TRIGGERS
...
class AssemblyStep:
    ...
    'One validated block-to-tray or final block-to-block instruction.'
    index: int
    block_color: Color
    tray_color: Color
    block_trigger: str
    tray_trigger: str
    destination_kind: EntityKind = EntityKind.TRAY
    ...
    @property
    def destination_color(self) -> Color:
        return self.tray_color

    @property
    def is_stack(self) -> bool:
        return self.destination_kind is EntityKind.BLOCK
    ...
    def from_colors(
        ...
        return cls(
            ...
            destination_kind=EntityKind.TRAY,
        )

    @classmethod
    def from_stack(
        cls, index: int, block_color: Color, target_block_color: Color
    ) -> "AssemblyStep":
        return cls(
            index=index,
            block_color=block_color,
            tray_color=target_block_color,
            block_trigger=block_color.block_trigger,
            tray_trigger=target_block_color.block_trigger,
            destination_kind=EntityKind.BLOCK,
        )
