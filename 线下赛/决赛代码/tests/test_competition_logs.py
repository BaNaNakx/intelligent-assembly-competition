...
class CompetitionLogTests(unittest.TestCase):
    def test_llm_log_keeps_only_task1_final_output(self) -> None:
        ...
        self.assertIn("步骤七：最后把粉色方块放在红色方块上", text)
        self.assertIn("红色物块装配位姿2", text)
    ...
    def test_export_creates_two_independent_text_files(self) -> None:
        ...
        store.record_vm(
            requested_at=now,
            received_at=now,
            request_text="start,11",
            response_text="#40;-40;40",
            status="成功",
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("#40;-40;40", result.vm_path.read_text(encoding="utf-8-sig"))
